...
router.post('/', verifyToken, async (req, res) => {
  ...
  try {
    const match = new LoLMatch(req.body);
    await match.save();
...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    const matchs = await LoLMatch.find(query)
      .limit(limit * 1)
      .skip((page - 1) * limit);
    const count = await LoLMatch.countDocuments(query);
...
router.get('/:id', async (req, res) => {
  try {
    const match = await LoLMatch.findById(req.params.id);
    if (!match) {
...
router.put('/:id', verifyToken, async (req, res) => {
  ...
  try {
    const match = await LoLMatch.findByIdAndUpdate(req.params.id, req.body, { new: true });
    if (!match) {
      return res.status(404).json({ message: 'Match non trouvé.' });
    }
...
router.delete('/:id', verifyToken, async (req, res) => {
  ...
  try {
    const match = await LoLMatch.findByIdAndDelete(req.params.id);
    if (!match) {
      return res.status(404).json({ message: 'Match non trouvé.' });
    }
